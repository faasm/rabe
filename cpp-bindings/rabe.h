...
int32_t rabe_bsw_encrypt(CpAbeContext *ctx, const char *policy,
                         const char *policyLanguage, const uint8_t *plainText,
                         size_t plainTextLen, BufferFfi **cipherText);

int32_t rabe_bsw_decrypt(CpAbeSecretKey *secretKey,
                         // WARNING: cipherText must be null-terminated
                         // (consider treating it as a string on C++, make sure
                         // we understand why Rabe uses it like a string)
...
class CpAbeContextWrapper {
  public:
    ...
    static CpAbeContextWrapper &
    get(const ContextFetchMode &fetchMode,
        ...
        if (fetchMode == ContextFetchMode::Create) {
            ...
            if (bytes.empty()) {
                throw std::runtime_error(
                    "Must provide bytes in this fetch mode");
            }
    ...
    // Encrypt plain-text with an access-structure over the policy. The policy
    // is a string where sub-string values are and-ed or or-ed like: std::string
    // policy = "\"A\" and \"B\"";
    std::vector<uint8_t> cpAbeEncrypt(const std::string &policy,
    ...
    std::vector<uint8_t>
    cpAbeDecrypt(const std::vector<std::string> &attributes,
