...
std::vector<uint8_t> encrypt(std::vector<uint8_t> key,
    ...
    size_t cipherTextSize = TLESS_AES256GCM_AUTH_SIZE +
                            TLESS_AES256GCM_NONCE_SIZE + plainText.size();
    std::vector<uint8_t> cipherText(cipherTextSize);

    aes256gcm_encrypt(key.data(), key.size(), nonce.data(), nonce.size(),
...
std::vector<uint8_t> decrypt(std::vector<uint8_t> key,
    ...
    aes256gcm_decrypt(key.data(), key.size(), nonce.data(), nonce.size(),
                      cipherText.data(), cipherText.size(), plainText.data(),
                      plainText.size());
